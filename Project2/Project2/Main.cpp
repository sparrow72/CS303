...
int main()
{
	...
	int id = 0; //Default ID
	double cTime = 1, cTime1 = 1, cTime2 = 1, uTime = 1; //Current time hh.mm.ss
	...
	#pragma omp parallel sections private (cTime)
		...
		#pragma omp section
					elevator(cTime1, intIter1, intReqMap[1], uMap, extReqMap, id, fin, uTime);
		#pragma omp section
					elevator(cTime2, intIter2, intReqMap[2], uMap, extReqMap, id, fin, uTime);
			}
			cTime++;
		}
...
void userOutput(map<int, User>& uMap)
{
	...
	{ //Output the map
        ...
        if (hour > 0 && sec < 10) {
            ...
            fout << (*iter).first << "\t\t\t" << sec << "sec" << "\t\t\t" << setw(6) <<
                (*iter).second.getStartFloor() << setw(5) << "\t\t" << (*iter).second.getDestFloor() << endl;
        }
	}
